...
<div>
	...
	<!--选择之前的pdf 部分-->	
		...
		<!-- 根据后台能传来的信息进行改变下面的div -->
			 <div id="choosePage">
				
			 </div>
				<form action="${pageContext.request.contextPath }/ModifyTask" method="get"  id="myform">
					   图书分配： <input type="text" name="phonenumber" id="phonenumber">
					    <input type="text" name="bookida" id="bookida" style="display:none">
					     <input type="text" name="flagpage" id="flagpage" style="display:none" value="${flagPage}"> 
...
function choosepage()
{
	if('${flagPage}'=='1')
	{
		$("#choosePage").innerHTML = ''; //每次显示时清空旧的内容
		var str=' <form action="" method="get"  id="myform">'
		   + '<select onchange="ajax_submit();" name="appname" id="selectBook">'
	       +'<option disabled selected value="0">==请选择已经上传未分配PDF识别任务进行分配==</option>'
	       +' </select>'
	  		+'</form>';
	  		$("#choosePage").append(str);
	}else if('${flagPage}'=='2')
		{
		$("#choosePage").innerHTML = ''; //每次显示时清空旧的内容
		var str=' <form action="" method="get"  id="myform">'
		   + '<select onchange="ajax_submit();" name="appname" id="selectBook">'
	       +'<option disabled selected value="0">==请选择已经分配的PDF识别任务进行重新分配==</option>'
	       +' </select>'
	  		+'</form>';
	  		$("#choosePage").append(str);
		}
