...
<div class="container">
	...
	<script>
		var eg = {};
		eg.data = ${photoListJson};
		//console.log(eg.data);
		eg.rootUrl = "/FTZ/";
		function toDetail(i){
			var photoSrc = eg.rootUrl + eg.data[i].recordUrl;
		...
		function change(obj){
			...
			for(var i = start;i < end && i < eg.data.length; i++) {
				...
				+	'<img style="width: 100%; height: 300px; display: block;" alt="100%x300" src="' + eg.rootUrl + eg.data[i].recordUrl +'" data-holder-rendered="true" title="' + eg.data[i].bookName + '">'
		...
		layui.use(['laypage'], function(){
			...
			var $ = layui.$, active = {
				    reload: function(){
				      ...
				      //执行重载
				      window.location.href="${pageContext.request.contextPath }/photoList?userId="+demoReload.val();
				    }
				  };
