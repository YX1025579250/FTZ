			...
			for(var i = start;i < end && i < eg.data.length; i++) {
				...
				var bookNameTrans = eg.data[i].bookName.length>10?eg.data[i].bookName.substr(0,10)+"..":eg.data[i].bookName;
					...
					+	'<div class="caption center">'
						...
						+	'<p><span>识别用户的手机号:</span><span>' +eg.data[i].phonenumber + '</span></p>'	
						+	'<p><span>书籍积分:</span><span>' +eg.data[i].bookMoney + '</span></p>'	
						+	'<p><span>页数:</span><span>' + eg.data[i].bookPage + '</span>&nbsp;&nbsp;'
						+	'</p>'
						+	'<p><a class="btn btn-primary" role="button" href="${pageContext.request.contextPath }/TaskMoneywaitDetail?bookId='+ eg.data[i].bookId +'">查看详情</a>&nbsp;&nbsp;'
						+	'<a class="btn btn-primary" role="button" href="${pageContext.request.contextPath }/MoneyGet?bookId='+ eg.data[i].bookId +'">给予积分</a></p>'
					+	'</div>'
					+	'</div>';
